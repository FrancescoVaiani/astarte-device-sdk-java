...
public class AstarteDeviceAggregateDatastreamInterface extends AstarteAggregateDatastreamInterface
  ...
  public void validatePayload(
    ...
    for(Map.Entry<String, AstarteInterfaceMapping> interfaceMappingEntry: getMappings().entrySet()){
      final AstarteInterfaceMapping astarteInterfaceMapping = interfaceMappingEntry.getValue();
      if(!payload.containsKey(astarteInterfaceMapping.getPath().substring((fomattedPath).length()))){
    ...
    for(Map.Entry<String, Object> data: payload.entrySet()){
      if(getMappings().containsKey(fomattedPath + data.getKey())){
        findMappingInInterface(fomattedPath + data.getKey()).validatePayload(data.getValue(), timestamp);
      } else{
        throw new AstarteInterfaceMappingNotFoundException(String.format("%s not found in interface", fomattedPath + data.getKey()));
      }
    }
